...
#set llm
llm=ChatOpenAI(model="gpt-4o", openai_api_key=oakey) ###NOTE: gpt-3.5-turbo-1106 would be cheaper!!! (GPT-3.5: 1$/1M tokens in & 2$/1M tokens out; GPT-4o:5$/1M tokens in & 15$/1M tokens out)
...
def build_retriever(docs):
    ...
    vectorstore=FAISS.from_documents(documents=docs, embedding=OpenAIEmbeddings(model="text-embedding-3-small", openai_api_key=oakey)) #expensive model: text-embedding-3-large
...
### SAVE RESULTS ###
df_results.to_excel(f"{directory}/Results_CiteRAG_export.xlsx")